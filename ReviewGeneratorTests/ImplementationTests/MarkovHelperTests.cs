...
{
    [TestClass]
    public class MarkovHelperTests
    {
        ...
        {
            MarkovHelper helper = new MarkovHelper();
            List<Review> reviewList = helper.Ingest(this.sampleData);

            Assert.IsNotNull(reviewList);
            Assert.IsTrue(reviewList.Count == 6);
            Assert.IsTrue(reviewList[0].GetType() == typeof(Review));
        }
        ...
        {
            MarkovHelper helper = new MarkovHelper();
            helper.Ingest(this.sampleData);
            helper.Train();

            Assert.IsNotNull(helper.Chain);
            Assert.IsTrue(helper.Chain.GetType() == typeof(MarkovChain<string>));
        }
    }
}
